...
function createProblemHTML(problem: Problem, showAnswer: boolean, interval: number, isReverse: boolean, isExample: boolean) {
    const clockHTML = createClock(problem.time, interval, isReverse && !isExample);
...
if (problemsContainer && answersContainer) {
    for (const [index, problem] of problemSet.entries()) {
        ...
        const showAnswer = isExample;

        const problemHTML = createProblemHTML(problem, showAnswer, config.interval, config.reverse, isExample);
        const answerHTML = createProblemHTML(problem, true, config.interval, config.reverse, true);
